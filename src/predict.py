...
class MultiHeadCOICOPPredictor:
    """Predictor class for multi-head COICOP classification."""

    def __init__(self, model_path: str | Path):
        """Initialize the predictor with a trained multi-head model.

        Args:
            model_path: Path to the saved multi-head classifier, or an MLflow
                artifact URI (runs:/, models:/, mlflow-artifacts:/)
        """
        model_path_str = str(model_path)
        if any(model_path_str.startswith(p) for p in ("runs:/", "models:/", "mlflow-artifacts:/")):
    ...
    def predict(
        ...
        Args:
            ...
            return_all_levels: Whether to include predictions at all levels.
            top_k: Number of top predictions per level.
            confidence_threshold: Minimum confidence per level.
